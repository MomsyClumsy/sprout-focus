...
fun TaskFormScreen(
    ...
    Column(
        ...
        // Подпись — вопросом от первого лица, а пример — внутри поля.
        // Форму глагола задают только эти два места: их видно до ввода.
        // Пояснение под полем на это не влияет — его читают уже после
        // того, как написали.
        //
        // Форма важна не сама по себе: этот текст встаёт во вторую половину
        // плана («…то найду ролик»). Переписать «найти» в «найду» приложение
        // не может — в русском это спряжение с чередованием корня.
        OutlinedTextField(
            value = firstStep,
            onValueChange = { firstStep = it },
            label = { Text("Первый шаг") },
            placeholder = { Text("открою файл и запишу заголовок") },
            supportingText = {
                Text("Настолько маленький шаг, чтобы отказаться было неловко")
            },
            modifier = Modifier.fillMaxWidth()
        )
