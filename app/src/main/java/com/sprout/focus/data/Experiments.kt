...
object Experiments {
    ...
    private fun shorter(f: Facts) = Hypothesis(
        ...
        measure = "Считаем, какая доля заходов доведена до конца.",
    )
    ...
    private fun ifThen(f: Facts) = Hypothesis(
        ...
        measure = "Считаем, к какой доле новых задач ты приступишь хотя бы раз.",
    )
    ...
    fun progressText(done: Int, total: Int, hypothesis: String): String = when {
        ...
        hypothesis == IF_THEN -> "Начато задач: $done из $total"
